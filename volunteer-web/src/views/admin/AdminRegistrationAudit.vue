  ...
  <div class="reg-manage-container">
    <el-card shadow="never" class="box-card" :body-style="{ padding: isMobile ? '10px' : '20px' }">
      ...
      <el-table
          ...
          v-loading="loading"
      >
        <el-table-column label="志愿者信息" width="220">
          <template #default="scope">
            <div style="display: flex; align-items: center; gap: 10px;">
              <el-avatar :size="32" :src="getFullAvatar(scope.row.avatar)" />
        ...
        <el-table-column label="报名活动与技能匹配" min-width="200">
          <template #default="scope">
            <div style="display: flex; flex-direction: column; gap: 4px;">
              <span style="font-weight: bold; font-size: 14px; color: #303133;">{{ scope.row.activityTitle }}</span>
              
              <!-- 技能匹配展示 (精修版) -->
              <div v-if="scope.row.requiredSkills" style="display: flex; flex-wrap: wrap; gap: 6px; margin-top: 4px;">
                <!-- 匹配项：深绿字，浅绿底，椭圆胶囊形 -->
                <div 
                  v-for="skill in getMatchingSkills(scope.row.userSkills, scope.row.requiredSkills)" 
                  :key="skill"
                  class="skill-pill-match"
                >
                  <el-icon style="margin-right: 3px; font-size: 14px;"><Check /></el-icon>{{ skill }}
                </div>
                
                <!-- 缺失项：浅灰，弱化显示 -->
                <div 
                  v-for="skill in getMissingSkills(scope.row.userSkills, scope.row.requiredSkills)" 
                  :key="skill"
      ...
      <div v-else class="mobile-list" v-loading="loading">
        <div v-for="item in regList" :key="item.regId" class="m-card">
          ...
          <div class="m-card-body">
            ...
            <!-- 技能匹配展示 (移动端) -->
            <div class="m-row" v-if="item.requiredSkills">
              <span class="m-label">技能匹配:</span>
              <div class="m-value" style="display: flex; flex-wrap: wrap; gap: 4px;">
                <span 
                  v-for="skill in getMatchingSkills(item.userSkills, item.requiredSkills)" 
                  :key="skill"
                  class="m-skill-match"
                >
                  ✓ {{ skill }}
                ...
                <span 
                  v-for="skill in getMissingSkills(item.userSkills, item.requiredSkills)" 
                  :key="skill"
                  class="m-skill-lack"
                >
                  {{ skill }}
...
import { Search, Stopwatch, Coin, CircleCheck, Check } from '@element-plus/icons-vue';
...
// --- 技能匹配逻辑 ---
const getMatchingSkills = (userSkillsJson, requiredSkillsJson) => {
  if (!userSkillsJson || !requiredSkillsJson) return [];
  try {
    const userSkills = JSON.parse(userSkillsJson);
    const requiredSkills = JSON.parse(requiredSkillsJson);
    return requiredSkills.filter(skill => userSkills.includes(skill));
  } catch (e) { return []; }
};

const getMissingSkills = (userSkillsJson, requiredSkillsJson) => {
  if (!requiredSkillsJson) return [];
  try {
    const userSkills = userSkillsJson ? JSON.parse(userSkillsJson) : [];
    const requiredSkills = JSON.parse(requiredSkillsJson);
    return requiredSkills.filter(skill => !userSkills.includes(skill));
  } catch (e) { return []; }
};
...
@media screen and (max-width: 768px) {
  ...
  .m-skill-match { color: #67c23a; font-weight: bold; }
  .m-skill-lack { color: #909399; text-decoration: line-through; opacity: 0.6; }
