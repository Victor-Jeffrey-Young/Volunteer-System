...
>
> `concurrency/` 三个类是真库 + 真多线程（`CountDownLatch` 同时起跑，5 线程）的回归测试，
> 专门锁死这几类竞态：并发报名只落一条、并发取消/驳回只释放一个名额、并发签到签退只生效一次、
> 并发兑换只成交一单、管理端编辑不得覆盖并发期间的报名数与库存。
> 另有 `LockTimeoutTest` 把连接池的 `innodb_lock_wait_timeout` 压到 1 秒，
> 验证真实的锁等待超时会被翻译成 409 而不是 500。用例自建数据并在结束后清理
> （会重算全局快照的排名用例还会把别人的名次原样放回）。
